...
def update_genes_with_gene_set(
    location_gene_set: str | Path, df: pd.DataFrame
) -> pd.DataFrame:
    ...
    symbol2ensg: dict[str, str] = hgnc.set_index("symbol")[
        "ensembl_gene_id"
    ].to_dict()
    ...
    for g in new_df["gene"].unique():
        ...
        if new != g:
            ...
            old_to_new[g] = new
    if old_to_new:
        new_df["gene"] = new_df["gene"].map(
            lambda g: old_to_new.get(g, g)
        )
    ...
    mask = ens_mapped.notna() & (
        new_df["ensembl_gene_id"] != ens_mapped
    )

    for g in new_df.loc[mask, "gene"].unique():
        old_ens = new_df.loc[
            (mask) & (new_df["gene"] == g), "ensembl_gene_id"
        ].unique()[0]
    ...
    if "variant" in new_df.columns and old_to_new:
        repl = new_df["gene"] + " "
        new_df["variant"] = repl + new_df["variant"].str.replace(
            r"^[^ ]+ ", "", regex=True
        )

    # --- warn about unmapped ------------------------------------
    unmapped = (
        set(new_df["gene"].unique()) - approved - set(alias2sym)
    )
    if unmapped:
        logger.warning(
            f"{len(unmapped)} genes could not be mapped: "
            f"{sorted(unmapped)}"
        )
