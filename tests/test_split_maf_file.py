...
_HEADER = "\t".join([
    "Hugo_Symbol", "Tumor_Sample_Barcode", "Variant_Type",
...
_MAF_CONTENT = _COMMENT_LINES + _HEADER + "\n" + "\n".join(_ROWS) + "\n"
...
def test_creates_one_file_per_sample(maf_file, tmp_path):
    ...
    assert produced == ["SAMPLE_A.maf", "SAMPLE_B.maf", "SAMPLE_C.maf"]
...
def test_each_file_contains_only_its_sample(maf_file, tmp_path):
    ...
    for sample in ("SAMPLE_A", "SAMPLE_B", "SAMPLE_C"):
        ...
        # header + one data row
        assert len(lines) == 2
        assert all(sample in l or l.startswith("Hugo_Symbol")
                   for l in lines)
